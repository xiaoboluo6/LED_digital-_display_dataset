...
class FontColor(object):
    ...
    def sample_from_data(self, bg_mat,flag,isLED):
        """
        ...
        # 修改颜色 修改修改 文字颜色 我直接输出试一下 下面那个if-else直接不管了
        if(flag == True and isLED == True):  # LED底纹
            col2 = np.array([176, 170, 169])
            return (col2, col1)
        elif(flag == False and isLED == True): # LED数字
            lvse = np.array([0, 255, 0])
            hongse = np.array([255, 0, 0])
            # lanhui = np.array([58, 78, 115]) # old
            lanhui = np.array([38, 58, 95])
            baise = np.array([255, 255, 255])
            arrays = [lvse, hongse, lanhui,baise]
            col2 = random.choice(arrays)
            return (col2, col1)
        elif(flag == False and isLED == False): # 普通数字
            baise = np.array([255,255,255])
            heise = np.array([0,0,0])
            lvse = np.array([20,186,168])
            arrays=[baise,heise,lvse]
            col2 = random.choice(arrays)
            return (col2, col1)
...
class Colorize(object):
    ...
    def __init__(self, model_dir='data'):#, im_path):
        ...
        # self.p_drop_shadow = -1    # 修改修改修改 shadow
        # self.p_border = 0.05   #
        # self.p_texture = 0.0 # use an image for coloring text

        # 修改修改修改 color自定义
        # probabilities of different text-effects:
        self.p_bevel = 0  # add bevel effect to text
        self.p_outline = 0  # just keep the outline of the text
        self.p_drop_shadow = -1  # 修改修改修改 shadow
        self.p_border = 0  #
        self.p_texture = 0.0  # use an image for coloring text
    ...
    def merge_two(self,fore,back,blend_type=None):
        ...
        # 1120颜色修改  下面一行我自己加了玩的

        if blend_type != None:
            c_blend = self.blend(c_f, c_b, blend_type)    # c_blend就是c_f
            # 原来的
    ...
    def color_text(self, text_arr, h, bg_arr,flag,isLED):
        """
        ...
        fg_col,bg_col = self.font_color.sample_from_data(bg_arr,flag,isLED)
        return Layer(alpha=text_arr, color=fg_col), fg_col, bg_col

    # 这里的flag表示是否是底部数码馆 一般为false
    def process(self, text_arr, bg_arr, min_h,flag = False,isLED = False):
        """
        ...
        """
        # decide on a color for the text:
        l_text, fg_col, bg_col = self.color_text(text_arr, min_h, bg_arr,flag,isLED)
        bg_col = np.mean(np.mean(bg_arr,axis=0),axis=0)
        l_bg = Layer(alpha=255*np.ones_like(text_arr,'uint8'),color=bg_col)

        # 1120 alpha控制
        l_text.alpha = l_text.alpha * np.clip(0.88 + 0.1*np.random.randn(), 0.85, 1.0)  # a_min本来是0.72
        layers = [l_text]
        blends = []
    ...
    def color(self, bg_arr, text_arr, hs, place_order=None, pad=20,flag = False,isLED = False):
        """
        ...
        for i in place_order[::-1]:
            ...
            # flag是用来判断是不是led阴影的  isled isnotled
            rdr0 = self.process(text_patch, bg, hs[i],flag,isLED)
