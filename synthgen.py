...
def get_text_placement_mask_WithoutRotate1120(xyz, mask, plane, pad=2, viz=False):
    """
    Returns a binary mask in which text can be placed.
    Also returns a homography from the original image
    ...
    for i in range(len(contour)):
        ...
        # R = su.rot3d(plane[:3], n_front)  # 删除或注释掉这一行，取消旋转
        # xyz = xyz.dot(R.T)  # 删除或注释掉这一行，取消旋转
        pts_fp.append(xyz[:, :2])
        pts.append(cnt_ij)

    # No rotation in 2D plane
    # rect = cv2.minAreaRect(pts_fp[0].copy().astype('float32'))
    # box = np.array(cv2.boxPoints(rect))
    # R2d = su.unrotate2d(box.copy())

    # Use the original contour points without rotation
    box = np.array(pts[0])
    pts_fp[0] = box.copy()

    # rescale the unrotated 2d points to approximately
    # the same scale as the target region:
    s = rescale_frontoparallel(pts_fp[0], box, pts[0])  # 修改这一行，使用原始轮廓点进行缩放
    box *= s
    pts_fp[0] *= s

    # paint the unrotated contour points:
    minxy = -np.min(box, axis=0) + pad // 2
    ROW = np.max(ssd.pdist(np.atleast_2d(box[:, 0]).T))
    COL = np.max(ssd.pdist(np.atleast_2d(box[:, 1]).T))

    place_mask = 255 * np.ones((int(np.ceil(COL)) + pad, int(np.ceil(ROW)) + pad), 'uint8')

    pts_fp_i32 = (pts_fp[0] + minxy[None, :]).astype('int32')

    cv2.drawContours(place_mask, [pts_fp_i32], -1, 0, thickness=cv2.FILLED, lineType=cv2.LINE_8)
    ...
    H, _ = cv2.findHomography(pts[0].astype('float32').copy(),
                              pts_fp_i32.astype('float32').copy(),
                              method=0)

    Hinv, _ = cv2.findHomography(pts_fp_i32.astype('float32').copy(),
    ...
    if viz:
        ...
        plt.scatter(pts_fp_i32[:, 0], pts_fp_i32[:, 1],
...
# 没改完 代码有问题的
def get_text_placement_mask_smallRotate1124(xyz, mask, plane, pad=2, max_rotation=10, viz=False):
    """
    Returns a binary mask in which text can be placed.
    ...
    """
    contour, hier = cv2.findContours(mask.copy().astype('uint8'), mode=cv2.RETR_CCOMP, method=cv2.CHAIN_APPROX_SIMPLE)[-2:]
    contour = [np.squeeze(c).astype('float') for c in contour]
    H, W = mask.shape[:2]
    ...
    # Apply a small random rotation within the specified range
    rotation_angle = random.uniform(-max_rotation, max_rotation)
    R = su.rot2d(rotation_angle)

    for i in range(len(pts_fp)):
        pts_fp[i] = (pts_fp[i] - center).dot(R.T) + center

    # Rescale the rotated 2D points to approximately
    # the same scale as the target region:
    s = rescale_frontoparallel(pts_fp[0], pts[0], pts[0])
    pts_fp[0] *= s

    # Paint the rotated contour points:
    minxy = -np.min(pts_fp[0], axis=0) + pad // 2
    ROW = np.max(ssd.pdist(np.atleast_2d(pts_fp[0][:, 0]).T))
    COL = np.max(ssd.pdist(np.atleast_2d(pts_fp[0][:, 1]).T))
    ...
    if viz:
        ...
        plt.subplot(1, 2, 2)
        plt.imshow(~place_mask)
        plt.scatter(pts_fp_i32[:, 0], pts_fp_i32[:, 1], edgecolors='none', facecolor='g', alpha=0.5)
        plt.show()
...
class RendererV3(object):
    ...
    def filter_for_placement(self,xyz,seg,regions):
        ...
        masks,Hs,Hinvs = [],[], []
        for idx,l in enumerate(regions['label']):
            res = get_text_placement_mask_WithoutRotate1120(xyz,seg==l,regions['coeff'][idx],pad=2)
            if res is not None:
                mask,H,Hinv = res
                masks.append(mask)
                Hs.append(H)
    ...
    def place_text(self,rgb,collision_mask,H,Hinv):
        ...
        if render_res is None: # rendering not successful
            ...
            # 修改修改修改
            text_mask,loc,bb,text,text_mask_88888 = render_res
            collision_mask += (255 * (text_mask_88888 > 0)).astype('uint8')
            text_mask_88888 = self.warpHomography(text_mask_88888, H, rgb.shape[:2][::-1])
        ...
        # 1120羽化 我直接取消羽化
        # text_mask_88888 = self.feather(text_mask_88888, min_h)

        # 1109修改 这里就要区分到底是生成普通的数字还是电子称样式的数字
        import random
        # Font('data/fonts/LED1/DS-DIGI.TTF') 和 Font('data/fonts/Normalnum1/FUTRFW.TTF')
        path = font.path
        ziti = path.split("/")[-1].split(".")[0]
        if ziti == "DS-DIGI": # 这是数码管数字
            # 数码管普部分
            # 以百分之70的概率 添加底部88888的阴影
            random_number = random.random() # 0-1随机数
            if random_number < 0.7:
                # 先画一个88888  flag表示这是底部阴影
                rgb = self.colorizer.color(rgb, [text_mask_88888], np.array([min_h]),flag=True,isLED=True)    # 表示LED阴影
                # 再画正常的数字
                im_final = self.colorizer.color(rgb,[text_mask],np.array([min_h]),flag=False,isLED=True)      # 表示LED数字
            else:
                # 直接画正常的数字
                im_final = self.colorizer.color(rgb, [text_mask], np.array([min_h]),flag=False,isLED=True)    # 表示LED数字
        if ziti == "FUTRFW" or ziti == "monofontorg": # 这是普通数字
            # 普通数字 正常画就行
            im_final = self.colorizer.color(rgb, [text_mask], np.array([min_h]),flag=False,isLED=False)   # 表示zhengchang数字
