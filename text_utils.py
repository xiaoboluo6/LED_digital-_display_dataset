...
def crop_safe(arr, rect, bbs=[], pad=0):
    ...
    v1 = [min(arr.shape[0], rect[0]+rect[2]+4*pad), min(arr.shape[1], rect[1]+rect[3])]
...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        # 修改修改修改修改修改修改
        self.min_nchar = 2
        self.min_font_h = 35 #px : 0.6*12 ~ 7px <= actual minimum height   16 -》40  不能太小 太小容易出错
        self.max_font_h = 180 #px      120-》180
        self.p_flat = 0.10

        # curved baseline:
        self.p_curved = -1      # 修改这里我改成了-1 这样能保证不会产生弯曲的数字
        ...
        # 修改修改修改修改修改修改 更换生成文本的内容
        self.text_source = TextSource(min_nchar=self.min_nchar,
                                      fn=osp.join(data_dir,'newsgroup/allzhengshu.txt'))
    ...
    # 电子称普通数字
    def render_multiline(self,font,text,fsize):
        """
        ...
        bbs = []
        space = font.get_rect('8') # rect(3,34,20,34)
        x, y = 0, 0
        for l in lines:
            # oldold
            x = 0
            ...
            path = font.path
            ziti = path.split("/")[-1].split(".")[0]
            # 修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改修改
            # 这里最好根据font的path进行修改 font.path
            # LED1/DS-DIGI.TTF
            # LED2/DS-DIGII.TTF
            # LED3/DS-DIGIT.TTF
            # Normalnum1/FUTRFW.TTF
            # Normalnum2/monofontorg.ttf
            ...
            if (ziti == "DS-DIGI"):
                for ch in l:  # render each character
                    if ch.isspace():  # just shift
                        ...
                        # 对于1的情况 首先应该提前加一个身位
                        if (ch == "1"):
                            ch_bounds = font.render_to(surf, (x + space.width - space.x * 2, y), ch)
                        elif (ch == "-"):
                            ch_bounds = font.render_to(surf, (x + space.x, y), ch)
                        else:
                        ...
                        # ----------------修改----------------
                        if (ch == "1"):
                            x += round(space.width * 1.3)
                        else:
                            x += round(space.width * 1.3)
                        bbs.append(np.array(ch_bounds))

            if (ziti == "FUTRFW" or ziti == "monofontorg"):
                space = font.get_rect('8')
                for ch in l:  # render each character
                    if ch.isspace():  # just shift
                        x += space.width
                    else:
                        ...
                        ch_bounds.y = y - ch_bounds.y
                        if (ch == "1" or ch == "-"):
                            x += round(space.width * 1.15)
                        else:
                            x += round(space.width * 1.1)
                        bbs.append(np.array(ch_bounds))
        ...
        # # 1201修改 为了保留空白的部分 x开始的位置我直接shoudongshezhi
        # rect_union.x = space.x
        # rect_union.width = bbs[-1][0] + bbs[-1][2] - rect_union.x
    ...
    # 电子称88888
    # 修改修改 我自己的render_multiline_Myself1011
    def render_multiline_Myself1011(self, font, text):
        """
        renders multiline TEXT on the pygame surface SURF with the
        font style FONT.
        A new line in text is denoted by \n, no other characters are
        ...
        # self.visualize_bb(surf_arr,bbs)
        return surf_arr, words, bbs, fsize

    # 修改1012
    # 电子称普通数字
    def render_curved(self, font, word_text, fsize):
        """
        ...
        # 修改0915自己添加的 我想要取消弯曲状的数字
        return self.render_multiline(font, word_text, fsize)
    ...
    # 修改 修改 我自己的 render_curved_Myself1011
    # 电子称88888
    def render_curved_Myself1011(self, font, word_text):
        """
        use curved baseline for rendering word
        """
        wl = len(word_text)
        ...
        # 修改0915自己添加的 我想要取消弯曲状的数字
        return self.render_multiline_Myself1011(font, word_text)
    ...
    # 修改修改修改修改 1011 我自己写一个place_text
    def place_text_1011Myself(self, text_arrs, back_arr, bbs,origin_loc):
        order = [0]
        ...
        for i in order:
            ...
            # 修改 越界
            w, h = text_arrs[i].shape
            if loc[0]+w > back_arr.shape[0] or loc[1]+h > back_arr.shape[1]:
                out_arr[loc[0]:loc[0] + w, loc[1]:loc[1] + h] += text_arrs[i][:back_arr.shape[0]-loc[0],:back_arr.shape[1]-loc[1]]
            else:
                out_arr[loc[0]:loc[0]+w,loc[1]:loc[1]+h] += text_arrs[i]
    ...
    def render_sample(self,font,mask):
        ...
        while i < self.max_shrink_trials and max_font_h > self.min_font_h:
            ...
            # 1011修改修改修改
            # 首先计算以下text的长度  决定一下用几位的8
            # 1201
            text_88888 = '8'* (len(text)+1)

            txt_arr_88888, txt_88888, bb_88888, fsize = self.render_curved_Myself1011(font, text_88888)
            txt_arr, txt, bb = self.render_curved(font, text, fsize)
            # 1011修改修改修改

            bb = self.bb_xywh2coords(bb)
            bb_88888 = self.bb_xywh2coords(bb_88888)

            space = font.get_rect('8')

            # make sure that the text-array is not bigger than mask array:
            if np.any(np.r_[txt_arr.shape[:2]] > np.r_[mask.shape[:2]]) and np.any(np.r_[txt_arr_88888.shape[:2]] > np.r_[mask.shape[:2]]):
            ...
            # 1011修改修改修改
            text_mask_88888, loc, bb_88888, _ = self.place_text([txt_arr_88888], mask, [bb_88888])
            if len(loc) > 0:
                # 1201 tianjia
                loc[0][1] += round(space.width * 1.3)

                text_mask, loc, bb, _ = self.place_text_1011Myself([txt_arr], mask, [bb],loc[0])

            if len(loc) > 0:#successful in placing the text collision-free:
                # return text_mask,loc[0],bb[0],text

                # # 1011修改修改修改
                # return text_mask,loc[0],bb_88888[0],text,text_mask_88888

                # 1201测试
                return text_mask,loc[0],bb_88888[0][:,:,1:],text,text_mask_88888
...
class FontState(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        #  修改点  修改以下models/num_font_px2pt10000.cp
        char_freq_path = osp.join(data_dir, 'models/char_freq.cp')        
        font_model_path = osp.join(data_dir, 'models/num_font_px2pt10000.cp')
    ...
    def sample(self):
        ...
        # # 数码管0.5   正常数字细体粗体都是0.25
        # final_choice = random.choices(choices,weights=weights)[0]

        return {
            'font': self.fonts[int(np.random.randint(0, len(self.fonts)))],  # 原来是同概率随机选择
    ...
    def init_font(self,fs):
        ...
        # font.underline = False           # 这里直接给false 不要出现下划线
        ...
        # 修改修改修改修改修改修改  zitiziti
        # 这里我直接自己定义font了  原本的在上面
        font = freetype.Font(fs['font'], size=fs['size'])
        #font.underline = fs['underline']
        font.underline = False           # 这里直接给false 不要出现下划线
        font.underline_adjustment = fs['underline_adjustment']   # 添加强调属性
        font.strong = False    # 添加强调属性
        font.oblique = False   # 添加倾斜属性
        font.strength = 0.05   # 添加字体强度
        char_spacing = 1000  # 字符间距
        font.antialiased = True   # 抗锯齿
        font.origin = True
